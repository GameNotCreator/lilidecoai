    ...
    <form action={(formData) => void submit(formData)} className="product-form">
      <header className="app-page-head">
        <div>
          ...
          <p>Une image nette, des mesures réelles et un ancrage précis suffisent.</p>
        </div>
      ...
      <div className="product-form-grid">
        <div className="app-card">
          <div className="form-section-title">
            <span>01</span>
            <div><h2>Photo produit</h2><p>Fond uni ou transparent recommandé.</p></div>
          </div>
          <label className="product-upload">
            {preview ? (
              ...
              <>
                ...
                <span>JPEG, PNG ou WebP · min. 320 px</span>
              </>
            )}
            <input
              type="file"
          ...
          <div className="mask-note">
            <Upload size={17} />
            <span>
              Le MVP produit un masque déterministe sur fond clair. Le pinceau de correction est
              préparé comme prochaine extension GPU/SAM.
            </span>
          </div>
        </div>
        <div className="app-card form-fields">
          <div className="form-section-title">
            <span>02</span>
            <div><h2>Identité & dimensions</h2><p>Ces valeurs pilotent l’échelle, pas le modèle IA.</p></div>
          </div>
          <div className="field"><label htmlFor="name">Nom</label><input id="name" name="name" required placeholder="Vase Sienne" /></div>
          <div className="two-fields">
            <div className="field"><label htmlFor="sku">SKU</label><input id="sku" name="sku" placeholder="VAS-042" /></div>
            <div className="field"><label htmlFor="material">Matériau</label><input id="material" name="material" required placeholder="Céramique mate" /></div>
          </div>
          ...
          <div className="two-fields">
            <div className="field">
              ...
              <select id="placementType" name="placementType">
                <option value="table">Table</option><option value="shelf">Étagère</option>
                <option value="wall">Mur</option><option value="floor">Sol</option>
            ...
            <div className="field">
              ...
              <select id="reflectance" name="reflectance">
                <option value="matte">Mate</option><option value="satin">Satinée</option>
          ...
          <div className="field"><label htmlFor="buyUrl">Lien d’achat</label><input id="buyUrl" name="buyUrl" type="url" placeholder="https://boutique.example/objet" /></div>
          <div className="field"><label htmlFor="description">Description</label><textarea id="description" name="description" rows={3} placeholder="Description courte du catalogue…" /></div>
          <Button type="submit" disabled={busy}><Check size={17} /> Créer et préparer</Button>
        </div>
      </div>
    </form>
  );
  ...
  return (
    <div className="field">
      ...
      <div className="unit-input">
        <input id={name} name={name} type="number" required min="1" max="1000" step="0.1" />
        <span>cm</span>
      </div>
    </div>
  );
